...
- **Phase title:** `Secure MCP ingress and durable TTL start/stop lifecycle`.
- **Observable boundary:** Phase 2 is cloud-complete at `dbb4262b5541d0b89998864bb1fd61f6e872022f`. `zodex local setup` now creates/reconciles the fixed no-home-mount machine, installs one versioned root-owned `zodex-agent` network namespace with a dedicated veth and interface-scoped nftables policy, attaches `zodexd` and `zodex-prd` to that namespace with empty capability bounds, verifies topology/policy/public reader access/secret isolation, and records a ready state only after verification. `zodex local exec` is public operator-only guest-root exec outside the restricted namespace and refuses stale setup/network/home-mount state. No Local tunnel/start/stop command exists yet.
- **Current blockers:** none. A cloud agent can begin Phase 3 by implementing the restricted tunnel runtime plus host-side durable TTL lease supervisor against the now-stable Local setup/network/exec boundary. Actual Secure MCP Tunnel connection and expiry behavior remain deferred to Phase 7.
